...
function AttendanceColor({ cell }) {
  if (cell.getValue() === "present") {
    return (
      <div className="bg-green-400 w-16 p-1 rounded text-center">
        {cell.getValue()}
      </div>
    );
  } else if (cell.getValue() === "absent") {
    return (
      <div className="bg-red-400 w-16 p-1 rounded text-center">
        {cell.getValue()}
      </div>
    );
  } else if (cell.getValue() === "MRM") {
    return (
      <div className="bg-cyan-400 w-16 p-1 rounded text-center">
        {cell.getValue()}
      </div>
    );
  }
...
const ExpenceTables = ({year, month}) => {
  ...
  const handleSaveRowEdits = async ({ exitEditingMode, row, values }) => {
    ...
    const updatedRes = await post("/account/expence", {
      ...
      <MaterialReactTable
        ...
        renderTopToolbarCustomActions={({ table }) => (
          <Box sx={{ display: "flex", alignItems: "center" }}>
            <Box sx={{ display: "flex", gap: 2, p: "0.5rem" }}>
              <Button
                color="primary"
