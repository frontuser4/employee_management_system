...
export default function UpdateForm({ editData, setCloseUpdateform}) {
  ...
  const expenceId = `${data.empId}${dayjs(date.$d).format("YYYY")}${dayjs(date.$d).format("MM")}${dayjs(date.$d).format("DD")}`;
  ...
  async function UpdateData(updatedata) {
    const res = await update("/getput", data.empId, dayjs(date.$d).format("MM"), dayjs(date.$d).format("YYYY"), updatedata);
    console.log(res);
  }
